import { useEffect, useState } from "react";
import api from "../../services/api";

function StaffManagement() {
  const [staff, setStaff] = useState([]);
  const [departments, setDepartments] = useState([]);
  const [loading, setLoading] = useState(true);

  const [departmentId, setDepartmentId] = useState("");
  const [fullName, setFullName] = useState("");
  const [position, setPosition] = useState("");
  const [email, setEmail] = useState("");
  const [phone, setPhone] = useState("");
  const [biography, setBiography] = useState("");
  const [image, setImage] = useState(null);

  const [editingId, setEditingId] = useState(null);

  const fetchData = async () => {
    try {
      const [staffResponse, departmentResponse] = await Promise.all([
        api.get("/staff"),
        api.get("/departments"),
      ]);

      setStaff(staffResponse.data.data || staffResponse.data);
      setDepartments(departmentResponse.data.data || departmentResponse.data);
    } catch (error) {
      console.error("Failed to load staff:", error.response?.data || error);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchData();
  }, []);

  const handleSubmit = async (e) => {
    e.preventDefault();

    try {
      if (editingId) {
        await api.put(`/staff/${editingId}`, {
          department_id: departmentId,
          full_name: fullName,
          position,
          email,
          phone,
          biography,
          image: typeof image === "string" ? image : "",
        });
      } else {
        const formData = new FormData();

        formData.append("department_id", departmentId);
        formData.append("full_name", fullName);
        formData.append("position", position);
        formData.append("email", email);
        formData.append("phone", phone);
        formData.append("biography", biography);

        if (image) {
          formData.append("image", image);
        }

        await api.post("/staff", formData);
      }

      resetForm();
      await fetchData();
    } catch (error) {
      console.error("Failed to save staff:", error.response?.data || error);
    }
  };

  const handleEdit = (person) => {
    setEditingId(person.id);
    setDepartmentId(person.department_id);
    setFullName(person.full_name);
    setPosition(person.position || "");
    setEmail(person.email || "");
    setPhone(person.phone || "");
    setBiography(person.biography || "");
    setImage(person.image || "");
  };

  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this staff member?")) {
      return;
    }

    try {
      await api.delete(`/staff/${id}`);
      await fetchData();
    } catch (error) {
      console.error("Failed to delete staff:", error.response?.data || error);
    }
  };

  const resetForm = () => {
    setEditingId(null);
    setDepartmentId("");
    setFullName("");
    setPosition("");
    setEmail("");
    setPhone("");
    setBiography("");
    setImage(null);
  };

  if (loading) {
    return <p>Loading staff...</p>;
  }

  return (
    <div className="admin-staff">
      <h2>Staff Management</h2>

      <form onSubmit={handleSubmit}>
        <div>
          <label>Department</label>

          <select
            value={departmentId}
            onChange={(e) => setDepartmentId(e.target.value)}
            required
          >
            <option value="">Select Department</option>

            {departments.map((department) => (
              <option key={department.id} value={department.id}>
                {department.name}
              </option>
            ))}
          </select>
        </div>

        <div>
          <label>Full Name</label>

          <input
            type="text"
            value={fullName}
            onChange={(e) => setFullName(e.target.value)}
            required
          />
        </div>

        <div>
          <label>Position</label>

          <input
            type="text"
            value={position}
            onChange={(e) => setPosition(e.target.value)}
          />
        </div>

        <div>
          <label>Email</label>

          <input
            type="email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
          />
        </div>

        <div>
          <label>Phone</label>

          <input
            type="text"
            value={phone}
            onChange={(e) => setPhone(e.target.value)}
          />
        </div>

        <div>
          <label>Biography</label>

          <textarea
            rows="6"
            value={biography}
            onChange={(e) => setBiography(e.target.value)}
          />
        </div>

        {!editingId && (
          <div>
            <label>Image</label>

            <input
              type="file"
              accept="image/*"
              onChange={(e) => setImage(e.target.files[0])}
            />
          </div>
        )}

        <button type="submit">
          {editingId ? "Update Staff" : "Add Staff"}
        </button>

        {editingId && (
          <button type="button" onClick={resetForm}>
            Cancel
          </button>
        )}
      </form>

      <hr />

      <h3>Existing Staff</h3>

      {staff.length === 0 ? (
        <p>No staff found.</p>
      ) : (
        <div>
          {staff.map((person) => (
            <div key={person.id}>
              <h3>{person.full_name}</h3>

              <p>Department: {person.department_name}</p>
              <p>Position: {person.position}</p>
              <p>Email: {person.email}</p>
              <p>Phone: {person.phone}</p>

              {person.biography && <p>{person.biography}</p>}

              {person.image && <p>Image: {person.image}</p>}

              <button onClick={() => handleEdit(person)}>Edit</button>

              <button onClick={() => handleDelete(person.id)}>Delete</button>
            </div>
          ))}
        </div>
      )}
    </div>
  );
}

export default StaffManagement;
