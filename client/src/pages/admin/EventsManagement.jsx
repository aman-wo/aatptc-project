...
function EventsManagement() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      console.error("Failed to load events:", error.response?.data || error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingSlug) {
        await api.put(`/events/${editingSlug}`, {
          title,
          description,
  ...
  return (
    <div className="admin-events">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit">
          {editingSlug ? "Update Event" : "Add Event"}
        </button>
        ...
        <div>
          {events.map((event) => (
            <div key={event.id}>
              ...
              <button onClick={() => handleDelete(event.slug)}>Delete</button>
            </div>
          ))}
        </div>
      )}
