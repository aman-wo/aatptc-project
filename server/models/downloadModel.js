...
const getAllDownloads = (callback) => {
  db.query("SELECT * FROM downloads ORDER BY created_at DESC", callback);
};

const getDownloadById = (id, callback) => {
  db.query("SELECT * FROM downloads WHERE id = ?", [id], callback);
};
...
const deleteDownload = (id, callback) => {
  db.query("DELETE FROM downloads WHERE id = ?", [id], callback);
};
